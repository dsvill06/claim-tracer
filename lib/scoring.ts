...
function normalizeUrl(u:unknown):string{if(typeof u!=='string'||!u)return '';const t=u.trim();if(/^https?:\/\//i.test(t))return t;if(t.startsWith('//'))return 'https:'+t;if(t.includes('.'))return 'https://'+t;return '';}
export function normalizeJudge(value:unknown):JudgeResult { const v=(value&&typeof value==='object'?value:{}) as Record<string,unknown>; const sources=Array.isArray(v.sources_found)?v.sources_found:[]; const clean=sources.map((s:any)=>({url:normalizeUrl(s?.url),domain:typeof s?.domain==='string'?s.domain:'unknown',title:typeof s?.title==='string'&&s.title?s.title:undefined,snippet:typeof s?.snippet==='string'&&s.snippet?s.snippet.slice(0,120):undefined,type:['primary','secondary','aggregator'].includes(s?.type)?s.type:'aggregator',stance:['supports','disputes','mentions'].includes(s?.stance)?s.stance:'mentions'})).filter(s=>s.url).filter((s,i,a)=>a.findIndex(x=>x.domain===s.domain&&x.stance===s.stance)===i); return {sources_found:clean,directness:v.directness==='exact'||v.directness==='partial'?'exact'===v.directness?'exact':'partial':'tangential',independent_corroborations:Math.max(0,Number(v.independent_corroborations)||0),notes:typeof v.notes==='string'?v.notes:'Search results were inconclusive.'} }
...
export const splitterPrompt=`You extract checkable factual claims from text. Return ONLY valid JSON: {"claims":["..."]}. Rules: (1) Copy the claim as a verbatim sentence or phrase directly from the text — do NOT paraphrase or reword. (2) Each claim must be atomic and independently checkable against external sources. (3) Exclude: opinions, predictions, rhetoric, feelings, article metadata (dates, timestamps, author names, bylines, publisher names). Maximum 8 claims; return [] if none.`
