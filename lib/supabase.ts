...
export async function saveAnalysis(input_text:string,claims:unknown[],source_url?:string){const {data,error}=await getServiceSupabase().from('analyses').insert({input_text,claims,...(source_url?{source_url}:{})}).select('id').single();if(error)throw error;return data.id as string}
